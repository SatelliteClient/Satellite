...
public final class RenderUtil {//extends MinecraftInstance ????
	...
	public static void drawBlockBox(BlockPos blockPos, Color color, boolean outline) {
		...
		double x = blockPos.getX()+0.5 - mc.getRenderManager().viewerPosX;
		double y = blockPos.getY() - mc.getRenderManager().viewerPosY;
		double z = blockPos.getZ()+0.5 - mc.getRenderManager().viewerPosZ;
		...
		if (block != null) {
			EntityPlayerSP iEntityPlayerSP = mc.player;
			double posX = iEntityPlayerSP.lastTickPosX + (iEntityPlayerSP.posX - iEntityPlayerSP.lastTickPosX) * timer.partialTicks;
			double posY = iEntityPlayerSP.lastTickPosY + (iEntityPlayerSP.posX - iEntityPlayerSP.lastTickPosY) * timer.partialTicks;
			double posZ = iEntityPlayerSP.lastTickPosZ + (iEntityPlayerSP.posX - iEntityPlayerSP.lastTickPosZ) * timer.partialTicks;
			axisAlignedBB = block.getSelectedBoundingBox(mc.world.getBlockState(blockPos), mc.world, blockPos).expand(0.0020000000949949026D, 0.0020000000949949026D, 0.0020000000949949026D).offset(-posX, -posY, -posZ);
		} 
	...
	public static void drawEntityBox(Entity entity, Color color, boolean outline) {
		...
		disableGlCap(new int[] { 3553, 2929 });
		GL11.glDepthMask(false);
		double x = entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * timer.partialTicks - renderManager.viewerPosX;
		double y = entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * timer.partialTicks - renderManager.viewerPosY;
		double z = entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * timer.partialTicks - renderManager.viewerPosZ;
		AxisAlignedBB entityBox = entity.getEntityBoundingBox();
		AxisAlignedBB axisAlignedBB = new AxisAlignedBB(entityBox
				.minX - entity.posX + x - 0.05D, entityBox
				.minY - entity.posY + y, entityBox
				.minZ - entity.posZ + z - 0.05D, entityBox
				.maxX - entity.posX + x + 0.05D, entityBox
				.maxY - entity.posY + y + 0.15D, entityBox
				.maxZ - entity.posZ + z + 0.05D);
		if (outline) {
			GL11.glLineWidth(1.0F);
			enableGlCap(2848);
	...
	public static void renderNameTag(String string, double x, double y, double z) {
		...
		setGlCap(3042, true);
		GL11.glBlendFunc(770, 771);
		int width = mc.fontRendererObj.getStringWidth(string) / 2;
		drawRect(-width - 1, -1, width + 1, mc.fontRendererObj.FONT_HEIGHT, -2147483648);
